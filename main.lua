...
function QuickApp:onInit()
    ...
    self:trace('')
    self:trace('Netatmo CO2 sensor')
    self:trace('User:', self.config:getUsername())
    self:updateProperty('manufacturer', 'Netatmo')
    self:updateProperty('manufacturer', 'CO2 sensor')
    self:run()
    self:updateView("label2", "text", string.format(self.i18n:get('co2'), 0))
    self.prompted = false
end
...
function QuickApp:pullNetatmoData()
    ...
    local callback = function(response)
        ...
        for _, deviceModule in pairs(device.modules) do
            if deviceModule.type == "NAModule4" then
        ...
        if sensor ~= nil then
            ...
            self:trace('[' .. sensor['_id'] .. '] CO2 sensor information updated')
            self:updateView("label1", "text", string.format(self.i18n:get('last-update'), os.date('%Y-%m-%d %H:%M:%S')))
            self:updateView("label2", "text", string.format(self.i18n:get('co2'), sensor.dashboard_data.CO2))
            ...
            self:updateProperty('value', sensor.dashboard_data.CO2)
